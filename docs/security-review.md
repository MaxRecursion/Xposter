...
| iPhone dashboard auth | Protecting mutation routes could otherwise break same-origin dashboard actions from a phone. | Added a browser-local API-key prompt/retry flow using `localStorage`. |
